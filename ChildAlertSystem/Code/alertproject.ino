...
float driversenscalibration_factor = -1500;
...
IPAddress server(192,168,1,5);  // server addr
...
{
  Serial.println("Driver is not in seat for >1 min so send status as false");
  driversts = false; 
}
return driversts;  
}
...
void loop() {
  ...
  Serial.print("Driver sts :  ");
  Serial.print(driversts,DEC);
  Serial.println();
  Serial.print("Child sts :  ");
  ...
  if(childsts == true)
  {
    if(driversts == false)
    {
      ...
      sendAlert(alertval);
      Serial.println("Alert Sent Wait for 2 min before sending next alert ");
      delay(120000); /* delay the next alrt */
    }  
    else
    {
